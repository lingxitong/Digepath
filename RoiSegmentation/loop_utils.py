...
# import Udistributed_utils as utils
import Utils.distribute_utils as utils
import torch
import torch.nn.functional as F
import tqdm
import numpy as np
from .cal_metrics import calculate_miou_mdice
...
def criterion(inputs, target, loss_weight=None, num_classes: int = 2, dice: bool = True, ignore_index: int = -100):
    if dice is True:
        ...
        losses = dice_loss(inputs, dice_target, multiclass=True, ignore_index=ignore_index)
    return losses
...
def evaluate(model, data_loader, device, num_classes):
    ...
    with torch.no_grad():  
        for i, (image, target) in tqdm.tqdm(enumerate(data_loader)):
    ...
    miou, mdice = calculate_miou_mdice(all_output,all_target,num_classes)
