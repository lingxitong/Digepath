    ...
    if PFM_name == 'Gigapath':
        ...
        if frozen:
            for param in model.parameters():
                param.requires_grad = False
        return model
    elif PFM_name == 'Ctranspath':
        from ctrans import ctranspath
        model = ctranspath()
        model.head = nn.Identity()
        state_dict = torch.load(PFM_weights_path,weights_only=True)
...
class DecoderCup(nn.Module):
    def __init__(self, emb_dim, decoder_channels,is_14 = False,is_ctrans = False):
        super().__init__()
        head_channels = 512
        n_skip = 0
        ...
        if is_ctrans:
            blocks[-1] = DecoderBlock(in_channels[-1], out_channels[-1], skip_channels[-1], scale=4)
...
class PFM_Seg_Model(nn.Module):
    def __init__(self, PFM_name, PFM_weights_path, emb_dim, frozen_PFM = True, img_size=224, num_classes=2, zero_head=False, vis=False):
        ...
        if PFM_name == 'Virchow_v2':
            ...
            self.decoder = DecoderCup(emb_dim, self.decoder_channels,is_ctrans=True)
        else:
    ...
    def forward(self, x):
        ...
        elif self.PFM_name == 'Ctranspath':
            x = self.transformer.forward_patch_tokens_features(x)
        elif self.PFM_name == 'Conch_V1_5':
            x = self.transformer.trunk.forward_features(x)[:,1:,:]
        else:
